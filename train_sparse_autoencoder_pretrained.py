...
from utils.utils import EarlyStopMonitor, get_neighbor_finder, SparseAutoencoder
...
parser = argparse.ArgumentParser('TGN sparse autoencoder training')
...
# New sparse autoencoder specific arguments
...
for i in range(args.n_runs):
    results_path = f"results/{args.data}_{args.embedding_module}-{args.aggregator}-{args.memory_dim}-vanilla{args.hidden_dim}-sparse-autoencoder.pkl" if i > 0 else f"results/{args.data}_{args.embedding_module}-{args.aggregator}-{args.memory_dim}-vanilla{args.hidden_dim}-sparse-autoencoder.pkl"
    ...
    logger.info('Start training sparse autoencoder task')

    # Initialize sparse autoencoder with sparsity parameters
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for k in range(0, num_batch, args.backprop_every):
            ...
            for j in range(args.backprop_every):
                ...
                # Sparse autoencoder reconstruction (returns both reconstructed and encoded)
                reconstructed, encoded = autoencoder(input_representation)

                # Calc reconstruction loss
                reconstruction_loss = reconstruction_criterion(reconstructed, input_representation)
                
                # Calculate sparsity loss
                sparsity_loss = autoencoder.sparsity_loss(encoded)
                
                # Total loss combines reconstruction and sparsity
                total_loss = reconstruction_loss + args.sparsity_weight * sparsity_loss
                
                loss += total_loss
                sparsity_loss_total += sparsity_loss

            loss /= args.backprop_every
            sparsity_loss_total /= args.backprop_every

            loss.backward()
            autoencoder_optimizer.step()
            train_losses.append(loss.item())
            train_sparsity_loss_vals.append(sparsity_loss_total.item())
        ...
        with torch.no_grad():
            ...
            for k in range(val_num_batch):
                ...
                # Reconstruct
                val_reconstructed, val_encoded = autoencoder(val_input_representation)

                # Loss
                val_reconstruction_loss = reconstruction_criterion(val_reconstructed, val_input_representation)
                val_sparsity_loss = autoencoder.sparsity_loss(val_encoded)
                val_losses.append(val_reconstruction_loss.item())
                val_sparsity_loss_vals.append(val_sparsity_loss.item())

        avg_val_loss = np.mean(val_losses)
        avg_val_sparsity_loss = np.mean(val_sparsity_loss_vals)
        val_reconstruction_losses.append(avg_val_loss)
        val_sparsity_losses.append(avg_val_sparsity_loss)
        train_reconstruction_losses.append(np.mean(train_losses))
        train_sparsity_losses.append(np.mean(train_sparsity_loss_vals))
        ...
        logger.info('Train reconstruction loss: {:.6f}, sparsity loss: {:.6f}'.format(np.mean(train_losses), np.mean(train_sparsity_loss_vals)))
        logger.info('Val reconstruction loss: {:.6f}, sparsity loss: {:.6f}'.format(avg_val_loss, avg_val_sparsity_loss))

        # Early stopping based on total validation loss (reconstruction + sparsity)
        total_val_loss = avg_val_loss + args.sparsity_weight * avg_val_sparsity_loss
        if early_stopper.early_stop_check(-total_val_loss):  # Negative because EarlyStopMonitor expects higher=better
    ...
    with torch.no_grad():
        ...
        for k in range(test_num_batch):
            ...
            # Reconstruct
            test_reconstructed, test_encoded = autoencoder(test_input_representation)

            # Loss
            test_reconstruction_loss = reconstruction_criterion(test_reconstructed, test_input_representation)
            test_sparsity_loss = autoencoder.sparsity_loss(test_encoded)
            test_losses.append(test_reconstruction_loss.item())
            test_sparsity_loss_vals.append(test_sparsity_loss.item())

    avg_test_loss = np.mean(test_losses)
    avg_test_sparsity_loss = np.mean(test_sparsity_loss_vals)

    logger.info('Test reconstruction loss: {:.6f}, sparsity loss: {:.6f}'.format(avg_test_loss, avg_test_sparsity_loss))
    ...
    pickle.dump({
        ...
        "test_sparsity_loss": avg_test_sparsity_loss,
        "epoch_times": epoch_times,
        "total_epoch_times": total_epoch_times
    }, open(results_path, "wb"))

    logger.info('Saving TGN and Sparse Autoencoder models')
    if USE_MEMORY:
