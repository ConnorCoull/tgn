...
from utils.utils import MaliciousClassifier, MergeLayer
...
class TGN(torch.nn.Module):
    def __init__(
        ...
        # MLP to compute probability of an edge given two node embeddings
        self.affinity_score = MergeLayer(
            self.n_node_features, self.n_node_features, self.n_node_features, 1
        )

        # MLP to compute probability of an edge being malicious
        # input is the concatenation of the source and destination node embeddings, the edge features
        # self.mlp_malicious

        self.malicious_classifier = MaliciousClassifier(
            total_dim=self.n_node_features * 2 + self.n_edge_features,
            hidden_dim=128,
            dropout=0.1,
        )
