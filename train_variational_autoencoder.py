...
from utils.utils import EarlyStopMonitor, get_neighbor_finder, VariationalAutoencoder
...
parser = argparse.ArgumentParser('TGN variational autoencoder training')
...
MODEL_SAVE_PATH = f'./saved_models/{args.prefix}-{args.data}-vae.pth'
get_checkpoint_path = lambda epoch: f'./saved_checkpoints/{args.prefix}-{args.data}-vae-{epoch}.pth'
...
for i in range(args.n_runs):
    results_path = "results/{}_{}_vae.pkl".format(args.prefix, i) if i > 0 else "results/{}_vae.pkl".format(args.prefix)
    ...
    logger.info('Start training variational autoencoder task')

    # Initialize variational autoencoder with latent dimension
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for k in range(0, num_batch, args.backprop_every):
            ...
            for j in range(args.backprop_every):
                ...
                # VAE forward pass (returns reconstructed, mu, logvar)
                reconstructed, mu, logvar = autoencoder(input_representation)
                ...
                # Total VAE loss (ELBO = reconstruction + beta * KL divergence)
            ...
            kl_loss_total /= args.backprop_every

            loss.backward()
            autoencoder_optimizer.step()
            train_losses.append(loss.item())
            train_kl_loss_vals.append(kl_loss_total.item())
        ...
        with torch.no_grad():
            ...
            for k in range(val_num_batch):
                ...
                # VAE forward pass
                val_reconstructed, val_mu, val_logvar = autoencoder(val_input_representation)

                # Loss
                val_reconstruction_loss = reconstruction_criterion(val_reconstructed, val_input_representation)
                val_kl_loss = autoencoder.kl_divergence(val_mu, val_logvar)
                val_losses.append(val_reconstruction_loss.item())
                val_kl_loss_vals.append(val_kl_loss.item())

        avg_val_loss = np.mean(val_losses)
        avg_val_kl_loss = np.mean(val_kl_loss_vals)
        ...
        train_kl_losses.append(np.mean(train_kl_loss_vals))
        ...
        logger.info('Train reconstruction loss: {:.6f}, KL loss: {:.6f}'.format(np.mean(train_losses), np.mean(train_kl_loss_vals)))
        logger.info('Val reconstruction loss: {:.6f}, KL loss: {:.6f}'.format(avg_val_loss, avg_val_kl_loss))

        # Early stopping based on total validation loss (reconstruction + beta * KL)
        total_val_loss = avg_val_loss + args.beta * avg_val_kl_loss
    ...
    with torch.no_grad():
        ...
        for k in range(test_num_batch):
            ...
            # VAE forward pass
            test_reconstructed, test_mu, test_logvar = autoencoder(test_input_representation)

            # Loss
            test_reconstruction_loss = reconstruction_criterion(test_reconstructed, test_input_representation)
            test_kl_loss = autoencoder.kl_divergence(test_mu, test_logvar)
            test_losses.append(test_reconstruction_loss.item())
            test_kl_loss_vals.append(test_kl_loss.item())
    ...
    logger.info('Saving TGN and VAE models')
    if USE_MEMORY:
